...
Parser <- R6::R6Class("Parser",
                      public = list(
                        ...
                        initialize = function(bracketstages = a.D, is_prose = a.D, dracor_id = 'insert_id', dracor_lang = 'insert_lang') {
                          self$tree_root <- xml_new_root("TEI", ns = "http://www.tei-c.org/ns/1.0")
                          xml_set_attr(self$tree_root, a.I, dracor_id)
                          ...
                          xml_set_attr(self$current_lowest_div, a.A, 0)
              ...
              <availability status="free">
                <p>In the public domain.</p>
                        ...
                        add_title_to_header = function(header, line) {
                          B <- xml_find_first(header, a.E)
                          A <- xml_add_child(B, "title")
                          xml_set_attr(A, a.B, 'main')
                        ...
                        parse_lines = function(ezdramalines) {
                          ...
                          for (B in ezdramalines) {
                            if (startsWith(B, '@author')) {
                              ...
                              } else if (self$lasting_comment && grepl(a.K, B)) {
                                B <- gsub('(\\<\\!--|--\\>)', '', B)
                        ...
                        handle_line_with_markup = function(first_character, rest_of_line) {
                          ...
                          if (D == '$') {
                            ...
                            G <- xml_add_child(self$tree_root$front, a.L)
                            xml_add_child(G, B)
                            ...
                            xml_set_attr(C, a.A, self$get_div_level(B))
                            if (xml_attr(C, a.A) > xml_attr(self$current_lowest_div, a.A)) {
                              xml_add_child(self$current_lowest_div, C)
                            } else if (xml_attr(C, a.A) == xml_attr(self$current_lowest_div, a.A)) {
                        ...
                        post_process = function() {
                          D <- set()
                          self$add_cast_items()
                          xml_remove(xml_find_first(self$tree_root, "//body")[[a.A]])
                          for (C in xml_find_all(self$tree_root, "//sp")) {
                            self$post_process_sp(C)
                            if (a.J %in% xml_attrs(C)) {
                              D <- union(D, list(c(xml_attr(C, a.J), str_trim(xml_text(xml_find_first(C, "speaker"))))))
                            }
                          }
                          for (A in xml_find_all(self$tree_root, "//div")) {
                            if (xml_attr(A, a.A) == 0) {
                              xml_set_attr(A, a.A, NULL)
                            } else if (xml_attr(A, a.A) == 1) {
                              xml_set_attr(A, a.A, NULL)
                              xml_set_attr(A, a.B, 'act')
                            } else if (xml_attr(A, a.A) == 2) {
                              xml_set_attr(A, a.A, NULL)
                              xml_set_attr(A, a.B, 'scene')
                            } else if (xml_attr(A, a.A) == 3) {
                              xml_set_attr(A, a.A, NULL)
                              xml_set_attr(A, a.B, 'subscene')
                            }
                          }
                        ...
                        add_cast_items = function() {
                          A <- xml_find_first(self$tree_root, a.L)
                          if (!is.null(A)) {
                            F <- xml_text(A)
                            B <- strsplit(F, a.H)[[1]]
                            xml_remove(A)
            ...
            <change when="%s">DESCRIBE CHANGE</change>
                        ...
                        add_particdesc_to_header = function(set_of_char_pairs) {
                          ...
                          for (F in set_of_char_pairs) {
                            ...
                            xml_set_attr(A, a.I, str_trim(substring(F[1], 2)))
                            xml_set_attr(A, "sex", self$guess_gender_stupid(xml_attr(A, a.I)))
                        ...
                        transliterate_speaker_ids = function(sp, speaker) {
                          ...
                          if (grepl('[йцукенгшщзхъфывапролджэячсмитью]', tolower(B))) {
                            A <- tolower(CleanAfterTranslit(translit(B, 'uk', reversed = a.D)))
                            A <- str_trim(A)
                        ...
                        indent_dracor_style = function() {
                          ...
                          B <- list()
                          for (E in strsplit(A, a.H)[[1]]) {
                            F <- gsub('^( +)', '\\1\\1', E)
                            B <- c(B, F)
                          }
                          A <- paste(B, collapse = a.H)
                          read_xml(A)
                          return(A)
                        },
